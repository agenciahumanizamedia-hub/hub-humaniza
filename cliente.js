...
function stageProduction(p){let items=p.production?.items||[];return`<div class="stage ${p.productionStatus==='Bloqueado'?'locked':''}"><div class="stage-head"><div><h2>02 Desenvolvimento Criativo</h2><p>Visualização dos roteiros, carrosséis e estáticos. O checklist é interno da Humaniza.</p></div>${statusPill(p.productionStatus)}</div>${p.productionStatus==='Bloqueado'?'<p class="muted">Bloqueado até aprovação do planejamento.</p>':`${items.map(renderContentItem).join('')}<label>Observações gerais do desenvolvimento</label><textarea class="note" id="production_note" oninput="autoGrow(this)" placeholder="Escreva aqui ajustes gerais sobre os conteúdos.">${p.production?.note||''}</textarea><div class="actions"><button class="btn-green" onclick="approveProduction()">Aprovar desenvolvimento</button><button class="btn-yellow" onclick="requestAdjust('production')">Solicitar ajustes</button></div>`}</div>`}
function stageCalendar(p){return`<div class="stage ${p.calendarStatus==='Bloqueado'?'locked':''}"><div class="stage-head"><div><h2>03 Calendário Editorial</h2><p>Datas, formatos e temas organizados.</p></div>${statusPill(p.calendarStatus)}</div>${p.calendarStatus==='Bloqueado'?'<p class="muted">Bloqueado até aprovação do desenvolvimento.</p>':`${field('Calendário Editorial','calendar_content',p.calendar?.content)}<label>Observações do cliente</label><textarea class="note" id="calendar_note" oninput="autoGrow(this)" placeholder="Escreva aqui ajustes sobre o calendário.">${p.calendar?.note||''}</textarea><div class="actions"><button class="btn-green" onclick="approveCalendar()">Aprovar calendário</button><button class="btn-yellow" onclick="requestAdjust('calendar')">Solicitar ajustes</button></div>`}</div>`}
