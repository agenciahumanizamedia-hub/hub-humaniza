...
Object.assign(window,{openClientModal,openProjectModal,closeModals,saveClient,saveProject,selectClient,selectProject,deleteClient,copyClientLink,approveStrategic,approveProduction,approveCalendar,requestAdjust,autoGrow,render,addContentItem,removeContentItem,toggleChecklist,formatText,formatHighlight,clearFormat,setTextSize,normalizeEditor,pasteClean,releaseStage,saveDraft});
...
function stageApproval(p,isAdmin){
  ...
  return `<div class="stage ${blocked?'locked':''}">
    ...
    ${blocked?'<p class="muted">Esta etapa ainda não foi liberada pela agência.</p>':`
      ...
      ${isAdmin?`<div class="actions"><button class="btn-dark" onclick="saveDraft('aprovação final')">Salvar alterações</button><button class="btn-green" onclick="releaseStage('approval')">Liberar aprovação final ao cliente</button></div>`:''}
    `}
  </div>`;
}
