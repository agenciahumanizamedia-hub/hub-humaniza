...
Object.assign(window,{openClientModal,openProjectModal,closeModals,saveClient,saveProject,selectClient,selectProject,deleteClient,copyClientLink,approveStrategic,approveProduction,approveCalendar,requestAdjust,autoGrow,render,addContentItem,removeContentItem,toggleChecklist,formatText,formatHighlight,releaseStage});
...
function richField(label,id,value){
  return `<div class="content-box rich-box">
    ...
    <div class="rich-toolbar">
      ...
      <button type="button" onclick="formatText('foreColor','#5B56FF')">Roxo</button>
      <button type="button" onclick="formatHighlight()">Destaque</button>
    </div>
    <div class="rich-editor" id="${id}" contenteditable="true" oninput="this.dataset.changed='1'">${value||''}</div>
  </div>`;
}
...
function stageProduction(p,isAdmin){let items=p.production?.items||[];let blocked=!isAdmin&&p.productionStatus==='Bloqueado';return`<div class="stage ${blocked?'locked':''}"><div class="stage-head"><div><h2>02 Desenvolvimento Criativo</h2><p>${isAdmin?'Área liberada para a agência preparar antes da aprovação do cliente.':'Cards separados para roteiros, carrosséis e estáticos.'}</p></div>${statusPill(p.productionStatus)}</div>${blocked?'<p class="muted">Bloqueado até aprovação do planejamento.</p>':`<div class="content-actions"><button onclick="addContentItem('roteiro')">+ Adicionar roteiro</button><button onclick="addContentItem('carrossel')">+ Adicionar carrossel</button><button onclick="addContentItem('estatico')">+ Adicionar estático</button></div>${items.map(renderContentItem).join('')}<label>Observações gerais do desenvolvimento</label><textarea class="note" id="production_note" oninput="autoGrow(this)">${p.production?.note||''}</textarea><div class="actions"><button class="btn-green" onclick="releaseStage('production')">Liberar desenvolvimento ao cliente</button><button class="btn-green" onclick="releaseStage('calendar')">Liberar calendário ao cliente</button><button class="btn-yellow" onclick="requestAdjust('production')">Registrar ajustes</button></div>`}</div>`}
  ...
  return `<div class="stage ${blocked?'locked':''}">
    ...
    ${blocked?'<p class="muted">Esta etapa ainda não foi liberada pela agência.</p>':`
      <div class="content-box">
        ...
        ${isAdmin?`<div class="rich-toolbar"><button type="button" onclick="formatText('bold')">B</button><button type="button" onclick="formatText('foreColor','#5B56FF')">Roxo</button><button type="button" onclick="formatHighlight()">Destaque</button></div><div class="rich-editor" id="approval_notes" contenteditable="true">${p.approvalNotes||''}</div>`:`<div class="readonly-box">${p.approvalNotes||''}</div>`}
      </div>
